...
export const useAppStore = create<AppState>((set) => ({
  ...
  removeContext: (id) => set((state) => ({ contexts: state.contexts.filter(context => context.id !== id) })),
  setNextActions: (nextActions) => set({ nextActions }),
  updateNextAction: (updatedAction) => set((state) => ({
    nextActions: state.nextActions.map(action => 
      action.id === updatedAction.id ? updatedAction : action
    )
  })),
  removeNextAction: (id) => set((state) => ({ nextActions: state.nextActions.filter(action => action.id !== id) })),
  reviewItems: {
    inboxItems: [],
    nextActions: [],
    projects: [],
  ...
  reorderNextActions: (reorderedActions) => set((state) => ({
