...
export function NextActionsList({ initialNextActions, projects, contexts, userId }: NextActionListProps) {
  ...
  const filteredAndSortedActions = useMemo(() => {
    return nextActions
      .filter(action => 
        (filterProject ? action.project?.id === filterProject : true) &&
        (filterContext ? action.contexts.some(c => c.id === filterContext) : true) &&
        (searchTerm ? action.title.toLowerCase().includes(searchTerm.toLowerCase()) : true)
      )
      .sort((a, b) => {
        if (sortBy === 'priority') {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    reorderNextActions(items.map((item, index) => ({ ...item, priority: index })))
          ...
          <Select value={filterContext} onValueChange={setFilterContext}>
            ...
            <SelectContent>
              ...
              {contexts.map((context) => (
                <SelectItem key={context.id} value={context.id}>{context.name}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
